...
subroutine init_pos()
  ...
  real(dp) :: dx, dy, dz, dr, overlap_cut

  ! --------------------------------------------------------------
  ! Initialization overlap rule:
  ! use a modest excluded radius only to avoid catastrophic overlaps.
  ! Do NOT use rc here; that would be much too strict.
  ! --------------------------------------------------------------
  overlap_cut = 0.80_dp * sigma

  call init_random_seed(34)

  call random_number(x(1)); x(1) = x(1) * boxlength
  call random_number(y(1)); y(1) = y(1) * boxlength
  call random_number(z(1)); z(1) = z(1) * boxlength
  ...
  do i = 2, n
    ...
    do
      ...
      if (attempts > 100000) then
        write(*,*) 'Position initialization failed after too many attempts.'
        stop 1
      end if

      call random_number(x_try(i)); x_try(i) = x_try(i) * boxlength
      call random_number(y_try(i)); y_try(i) = y_try(i) * boxlength
      call random_number(z_try(i)); z_try(i) = z_try(i) * boxlength

      do j = 1, i-1
        dx = minimum_image(x_try(i) - x(j))
        dy = minimum_image(y_try(i) - y(j))
        dz = minimum_image(z_try(i) - z(j))
        dr = sqrt(dx*dx + dy*dy + dz*dz)
        if (dr < overlap_cut) exit
      end do
